...
def generate_optimizer_result(
        ...
        if gr_price / us_list < 0.32:
            recs.append(OptimizerRecommendation(
                ...
                rationale=(
                    ...
                    "SE, DK, and other referencing markets. Withdrawal could lift the cascade floor."
                ),
    ...
    candidates = [
        c for c in _US_REF_MARKETS
        if c in final_prices
        and c not in levers.delayed_launches
        and c not in levers.withdrawals
    ]
    ...
    if m1 and m2 and m2 > m1 and not regs.guard.submit_method_ii and regs.guard.active:
        recs.append(OptimizerRecommendation(
            ...
            rationale=(
                ...
                "no change. However, this is pre-emptive compliance that builds credibility with CMS."
            ),
